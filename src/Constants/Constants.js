// Routing Constants
export const ROUTE_HOME = "/";
export const ROUTE_MEAT_SELECT = "/meat";
export const ROUTE_VEG_SELECT = "/veg";
export const ROUTE_SUMMARY = "/summary";

// Local Storage for Meat
export const LOCAL_STORAGE_PORK = "Pork";
export const LOCAL_STORAGE_BEEF = "Beef";
export const LOCAL_STORAGE_CHICKEN_BREAST = "Chicken breast";
export const LOCAL_STORAGE_HAMBURGER = "Hamburger";
export const LOCAL_STORAGE_BURRITO = "Burrito";
export const LOCAL_STORAGE_MEAT_TOTAL = "meatSum";

// Local Storage for Veggie
export const LOCAL_STORAGE_VEGGIE_TOTAL = "veggieSum";

// Card Type
export const CARD_MEAT = "CardMeat";
export const CARD_VEGGIE = "CardVeggie";